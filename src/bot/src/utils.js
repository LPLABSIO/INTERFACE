...
function log(message, error = false) {
    ...
    if (typeof message === 'object') {
        ...
        // Forcer le flush immédiat pour l'interface
...
async function findAndTypeCharByChar(client, value, fast = false) {
    try {
        ...
        log(`Error typing characters for element ${selector}: ${error.message}`);
        return false;
    }
}
...
async function checkAndTerminateApp(client, bundleId, forceTerminate = false) {
    try {
        ...
        // Ne pas propager l'erreur si c'est une erreur de session
        if (error.message && error.message.includes('session is either terminated')) {
            log(`Session already terminated, continuing...`);
            return 1; // Return "Not running" state
        }
        throw error;
    }
}
